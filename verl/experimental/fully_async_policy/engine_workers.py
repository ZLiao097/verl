    ...
    def sync_rollout_weights_by_checkpoint(self, sync_group_name="actor_rollout"):
        ...
        if self._is_rollout and (not self._is_actor):
            if rollout_name == "vllm":
                ...
                if self.config.actor_rollout_ref.actor.strategy in ["fsdp", "fsdp2"]:
                    raise NotImplementedError(f"Fully async sglang backend does not support actor strategy: {self.config.actor_rollout_ref.actor.strategy}")  
...
class DetachActorWorker(DetachNcclSync):
    ...
    def copy_model_to_cpu(self, actor_module):
        if self.config.actor_rollout_ref.actor.strategy in ["fsdp", "fsdp2"]:
            from verl.experimental.fully_async_policy.fsdp2_utils import fsdp2_sharded_save_to_cpu

            return fsdp2_sharded_save_to_cpu(actor_module)
        elif self.config.actor_rollout_ref.actor.strategy == "megatron":
            from verl.experimental.fully_async_policy.megatron_utils import copy_megatron_model_to_cpu

            return copy_megatron_model_to_cpu(actor_module)
        else:
            raise NotImplementedError(f"Unsupported strategy: {self.config.actor_rollout_ref.actor.strategy}")
    ...
    def restore_model_from_cpu(self, n):
        if self.config.actor_rollout_ref.actor.strategy in ["fsdp", "fsdp2"]:
            ...
            from verl.experimental.fully_async_policy.megatron_utils import restore_megatron_model_from_cpu

            restore_megatron_model_from_cpu(self.actor.engine.module, self.cpu_saved_models[n])
        else:
            raise NotImplementedError(f"Unsupported strategy: {self.config.actor_rollout_ref.actor.strategy}")
    ...
    def save_model_to_cpu(self, n):
        ...
        self.cpu_saved_models[n] = self.copy_model_to_cpu(self.actor.engine.module)

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def restore_model_from_cpu(self, n):
        if n in self.cpu_saved_models:
            self.restore_model_from_cpu(n)
